...
For simplicity, we will focus on just the *Bugula* for now. There is also an issue with late-stage measurements, so we will follow Pettersen *et al.* and work just with the larval and early-stage *Bugula*.
...
This is called a *random intercepts model*, because we don't include any variables; we just have random variation around the intercept $\beta_0$.
...
```{r eval=F}
m1 <- lmer(Metabolic ~ (1|Run), data = bugula)
summary(m1)
```
...
$L(\widehat{\lambda}) = \prod \limits_{i=1}^n \dfrac{\lambda^{Y_i} e^{-\lambda}}{Y_i!}$
