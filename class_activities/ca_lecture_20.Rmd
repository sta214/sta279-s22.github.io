...
2. From a previous activity, we know that $\widehat{\rho}_{group} = 0.13$ . Would you expect the denominator degrees of freedom to be more similar to 41, or 1559?

To actually approximate the degrees of freedom, we will use the `lmerTest` package, which approximates degrees of freedom with Satterthwaite's method. Make sure the `lmerTest` package is installed, then run the following code:
...
3. What is the approximate denominator degrees of freedom, using Satterthwaite's method?

4. Do we have evidence for a relationship between overall satisfaction and price?

5. Whenever we do inference with a statistical model, we are making assumptions. For example, in linear regression we have assumptions like shape, constant variance, normality, independence, and randomness. What assumptions does our mixed effects model make?