...
1. Write down the likelihood $L(\widehat{\pi}_0)$, and the log likelihood $\log L(\widehat{\pi}_0)$. Recall the following rules for logarithms:

* $\log(xy) = \log(x) + \log(y)$
* $\log(x^y) = y \log(x)$

2. Modify the simulation code from the slides to calculate $L(\widehat{\pi}_0)$ for $\widehat{\pi}_0 = 0, 0.05, 0.1, 0.15, ..., 0.9, 0.95, 1$. For reference, the code from the slides is below. For what value of $\widehat{\pi}_0$ is the likelihood the highest?
...
3. Use calculus to maximize the likelihood from Exercise 1. Remember to differentiate the log likelihood, and recall the following rules for derivatives:
